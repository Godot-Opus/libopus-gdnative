...
PoolByteArray OpusEncoderNode::resample_441kh_48kh(const PoolByteArray &rawPcm)
{
	...
	const uint16_t *readSamples = reinterpret_cast<const uint16_t *>(reader.ptr());
	//const int numReadSamples = rawPcm.size() / 2;

	const int upsampledBytesSize = (int)floor(rawPcm.size() * ratio);
	const int numWriteSamples = upsampledBytesSize / 2;
	uint16_t *writableSamples = new uint16_t[numWriteSamples];
...
//	const int numWriteSamples = newSize / 2;
	...
	for(int xx=0; xx<numWriteSamples; xx+=channels)
	{
		...
		{
			writableSamples[xx] = (readSamples[map44X] + readSamples[map44XLast]) / 2;
			if (channels==2)
			{
				writableSamples[xx+1] = (readSamples[map44X+1] + readSamples[map44XLast+1]) / 2;
			}
		}
		else
		{
	...
	for(int ii=0; ii<upsampledBytesSize; ++ii)
	{
...
PoolByteArray OpusEncoderNode::encode(const PoolByteArray &rawPcm)
{
	...
	const PoolByteArray &upsampled = rawPcm;

	const int numPcmBytes = upsampled.size();
	const uint8_t *pcm_bytes = upsampled.read().ptr();
	...
	while(!done)
	{
		...
		for(int ii = markPos; ii < markPos + (curFrameSize * channels); ii++)
			inputSamples[ii] = pcm_bytes[2 * ii + 1] << 8 | pcm_bytes[2 * ii];
